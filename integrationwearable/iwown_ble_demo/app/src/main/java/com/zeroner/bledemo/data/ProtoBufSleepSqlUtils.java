...
public class ProtoBufSleepSqlUtils {
    ...
    public static String getRootPath(){
        ...
        if(SuperBleSDK.getInstance().getContext()!=null){
            if(SuperBleSDK.getInstance().getContext().getExternalCacheDir()!=null){
                sdRootPath = Objects.requireNonNull(SuperBleSDK.getInstance().getContext().getExternalCacheDir().getParentFile()).getAbsolutePath();
                return sdRootPath;
            }
        }
        return "";
    ...
    public static void testSleep(){
        DateUtil dateUtil = new DateUtil(2020,7,10);
        String rootPath = BaseActionUtils.FilePath.ProtoBuf_Ble_80_Sleep_Dir_Test;
        String fileName1 = "test01.json";
        String fileName2 = "test02.json";
        SA_SleepBufInfo sleep = getSleep(rootPath + fileName1, rootPath + fileName2, dateUtil, dataFrom);
    ...
    private static void disAndWriteData(final Set<SleepBean> sortTables) {
        if(sortTables==null || sortTables.size()==0){
            return;
        }
        Observable.zip(Observable.fromIterable(sortTables), Observable.interval(500, TimeUnit.MILLISECONDS),
    ...
    private static String filterJson(List<ProtoBuf_80_data> data) {
        ...
        for (ProtoBuf_80_data index : data) {
            File_protobuf_80data file_protobuf_80data = JsonTool.fromJson(index.getSleepData(),File_protobuf_80data.class);
    ...
    public static void dispSleepData(final int year, final int month, final int day) {
         ...
         */
//        dataFrom = PrefUtil.getString(BleApplication.getInstance(), BaseActionUtils.ACTION_DEVICE_NAME) + "";
        dataFrom = ProtoBufDataParsePersenter.getDataFrom();
        ...
        Observable.create(new ObservableOnSubscribe<SleepProtoData>() {
            ...
            public void subscribe(ObservableEmitter<SleepProtoData> emitter) throws Exception {
                ...
                if(!sleepProtoData.hasData()){
                    SA_SleepBufInfo sleepError = new SA_SleepBufInfo();
                    sleepError.completeFlag = -1;
                    EventBus.getDefault().post(sleepError);
                }
                ...
                .map(new Function<SleepProtoData, String>() {
                    ...
                    public String apply(SleepProtoData sleepProtoData) throws Exception {
                        ...
                        if(sleep!=null){
                            Log.i("guanfengjun","睡眠返回已发送 ");
                            EventBus.getDefault().post(sleep);
                        }else{
                            SA_SleepBufInfo sleepError = new SA_SleepBufInfo();
                            sleepError.completeFlag = -1;
                            EventBus.getDefault().post(sleepError);
    ...
    public static void localSleepDataToSleepFinal(SA_SleepBufInfo f1SleepData, String date) {
        if (null != f1SleepData.sleepdata) {
            ...
            if (sleepDataByDate1 == null) {
                ...
                sleepDataByDate1.setUid(testUid);
            }
    ...
    private static SA_SleepBufInfo getSleep(String yesDatePath, String toDatePath, DateUtil dateUtil, final String dataFrom) {
        try {
            ...
            File file = new File(getRootPath()+toDatePath);
            if (file.exists()) {
                ...
                int status = jni.calculateSleepFileWithAF(getRootPath() + yesDatePath, getRootPath() + toDatePath, dateUtil.getSyyyyMMddDate(), 1, yesRriData, rriData, retData);
