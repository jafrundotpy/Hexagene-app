...
public class SleepActivity extends AppCompatActivity {
    ...
    private void findByIdView() {
         toolbarDeviceSleep = findViewById(R.id.toolbar_device_sleep);
        cusSleepBtn = findViewById(R.id.cusSleepBtn);
        oldSleepView = findViewById(R.id.sleepOldView);
         sleepRecyclerView = findViewById(R.id.sleep_recyclerView);
         protuSleep = findViewById(R.id.protu_sleep);
        cusSleepBtn.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void cusTodaySleep(){
        oldSleepView.setText("重新计算中...");
        protuSleep.removeAllViews();
        DateUtil todayDate = new DateUtil();
        ProtoBufSleepSqlUtils.dispSleepData(todayDate.getYear(),todayDate.getMonth(),todayDate.getDay());
        ...
        if(sleep_final_data!=null && !TextUtils.isEmpty(sleep_final_data.getSleep_segment())){
            ...
            if(segList!=null){
                for (SleepSegment sleepSegment : segList) {
                    ...
                    long timestamp = stDa.getUnixTimestamp() + sleepSegment.getSt()*60;
                    DateUtil stFD = new DateUtil(timestamp,true);
                    String timeStr = ": "+ times + getString(R.string.sleep_minute);
                    TextView textView = new TextView(this);
                    String msg = "";
                    if(sleepSegment.getType()==3){
                        msg = getString(R.string.sleep_detail_deep_1) +": "+ timeStr ;
                    }else if(sleepSegment.getType()==4){
                        msg = getString(R.string.sleep_detail_light_1)+": " + timeStr;
                    }else if(sleepSegment.getType()==7){
                        msg = "Eye: " + timeStr;
                    }else{
                        msg = getString(R.string.sleep_detail_weak_1) +": "+ timeStr;
                    }
                    msg = msg +" & "+stFD.getHHmmDate();
    ...
    public void onSleepEventMainThread(SA_SleepBufInfo events) {
        ...
        if(events.outSleepTime!=null
            ...
            oldSleepView.setText(JsonTool.toJson(events));
            initProView();
        }else{
            oldSleepView.setText("今日没有睡眠: "+dateUtil.getY_M_D());
        }
    }
